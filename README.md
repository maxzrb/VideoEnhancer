...
1. 在**不限速+登录无需转存**就能下载的移动云网盘链接 https://yun.139.com/shareweb/#/w/i/2wFGxhFFvSYxf
   下载VideoEnhancer-CLI.7z，解压到不含中文的目录。

2. 把编译产物复制到主程序目录下的 Plugin 文件夹：

   ```
   <主程序目录>\Plugin\videoenhancer.3fui.dll
   ```

   （文件名必须以 `.3fui.dll` 结尾，宿主按 `*.3fui.dll` 扫描加载；程序集名保持 `videoenhancer`，入口类型为 `videoenhancer.Entry`。）

3. 启动主程序，左侧最下方出现「视频超分」页面。

4. 点击视频超分页面，更改videoenhancer.exe路径为你的路径。程序会自动检测环境是否正常，自动扫描模型。
...
1. 第一排「插件总开关」（布尔开关，最上方）。打开时若未指定过 videoenhancer.exe，
   会弹出文件选择框，请选择你刚才已经解压好的videoenhancer.exe路径。
...
3. 第三排「补帧开关」（布尔开关）——打开时启用 RIFE 补帧（**不可与超分同时开启**）；
