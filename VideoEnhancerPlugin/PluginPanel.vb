...
Namespace videoenhancer
    ...
    Public Class PluginPanel
        ...
        Private Sub OnUpscaleSwitchChanged(sender As Object, e As EventArgs)
            ...
            If _switchUpscale.Checked AndAlso _config.Backend = "cuda" Then
        ...
        Private Sub ApplyModelList(models As List(Of String))
            ...
            Else
                If _config.Backend = "cuda" AndAlso _config.UpscaleEnabled Then
                    _cmbModel.WaterText = "未找到 .pth 放大模型"
                    ShowStatus("未在 models 找到 .pth 放大模型，已回退到 NCNN 推理", True)
        ...
        Private Sub BuildUpscalePage()
            ...
            _pageUpscale.Padding = New Padding(0, 4, 0, 0)
        ...
        Private Sub BuildAdvancedPage()
            ...
            Dim sectionQuad As New Panel() With {.Dock = DockStyle.Top, .Height = 56, .BackColor = Color.Transparent, .Padding = New Padding(0, 10, 0, 0)}
            ...
            ' 说明文字
            Dim sectionDesc As New Panel() With {.Dock = DockStyle.Top, .Height = 84, .BackColor = Color.Transparent, .Padding = New Padding(2, 10, 0, 0)}
        ...
        Private Sub SyncBackendCombo()
            If _cmbBackend.Items.Count = 0 Then
                Return
            End If
            _cmbBackend.SelectedIndex = If(_config.Backend = "cuda", 1, 0)
        End Sub
